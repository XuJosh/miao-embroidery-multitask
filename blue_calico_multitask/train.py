"""
Training script for the Blue Calico multi-task network.

Supports two defect learning modes:
    --defect_mode seg   : pixel-wise segmentation (needs mask annotations)
    --defect_mode cls   : image-level classification (only 0/1 labels needed)
"""
...
from model import BlueCalicoNet, MultiTaskLoss
...
def main():
    ...
    parser.add_argument('--defect_mode', type=str, default='seg', choices=['seg', 'cls'],
                        help='seg=pixel mask, cls=image-level 0/1 label')
    ...
    model = BlueCalicoNet(num_patterns=5, img_size=args.input_size,
                          defect_mode=args.defect_mode).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if epoch % 5 == 0:
            torch.save(model.state_dict(),
                       os.path.join(args.save_dir, f'bluecalico_{args.defect_mode}_epoch{epoch}.pth'))

        if val_m['total'] < best_val_total:
            best_val_total = val_m['total']
            torch.save(model.state_dict(),
                       os.path.join(args.save_dir, f'bluecalico_{args.defect_mode}_best.pth'))
            print(f"  -> New best model saved (val_total={best_val_total:.4f})")

    # final test
    best_path = os.path.join(args.save_dir, f'bluecalico_{args.defect_mode}_best.pth')
