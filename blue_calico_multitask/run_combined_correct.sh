#!/usr/bin/env bash
# Train the combined ResNet50 + DINOv2/Transformer model on corrected data.
...
SAVE_DIR="checkpoints_combined_correct"
LOG_FILE="logs/train_combined_correct_150.log"
...
PY
        ...
        if [ "$EPOCH" -ge "$EPOCHS" ]; then
            echo "[$(date)] Combined model already completed $EPOCH epochs. Exiting."
            break
        fi
        echo "[$(date)] Attempt $attempt: resuming combined from $RESUME (epoch $EPOCH)"
    else
        echo "[$(date)] Attempt $attempt: starting combined from scratch"
    fi
    ...
    if [ "$EXIT_CODE" -eq 0 ]; then
        echo "[$(date)] Combined training exited cleanly."
        break
    fi

    echo "[$(date)] Combined exited with code $EXIT_CODE. Restarting in 30 seconds..."
    sleep 30
done
