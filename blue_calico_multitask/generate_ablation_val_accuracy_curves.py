...
def plot_val_accuracy_curves():
    """Plot validation accuracy curves for three tasks across ablation variants."""
    variants = [
    ...
    tasks = [
        ('Auth Accuracy', 'auth_acc', 'Auth Validation Accuracy'),
        ('Pattern Accuracy', 'pattern_acc', 'Pattern Validation Accuracy'),
        ('Defect Accuracy', 'defect_acc', 'Defect Validation Accuracy'),
    ]
    ...
    for col, (task_name, task_key, title) in enumerate(tasks):
        ...
        for name, path, color, linestyle, marker in variants:
            ...
            val_acc = np.array([h['val'][task_key] for h in history])
            val_smooth = smooth(val_acc, size=5)

            # Plot raw with low alpha
            ax.plot(epochs, val_acc, color=color, alpha=0.18, linewidth=1)
            # Plot smoothed
    ...
    fig.suptitle('Validation Accuracy Curves of Ablation Variants', fontsize=15)
    plt.tight_layout(rect=[0, 0.08, 1, 0.95])
    out_path = 'D:/song/kimi_DEMO/blue_calico_multitask/ablation_val_accuracy_curves.png'
    plt.savefig(out_path, dpi=300, bbox_inches='tight')
    plt.close()
    print(f'Saved validation accuracy curves to: {out_path}')
