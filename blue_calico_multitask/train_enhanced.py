"""
Training script for EmbroideryNetEnhanced.
Adds: frequency branch, ArcFace heads, dynamic multi-task loss.
"""
...
from model_enhanced import EmbroideryNetEnhanced, DynamicMultiTaskLoss
...
def evaluate(model, loader, criterion, device, defect_mode, desc='Val'):
    ...
    for batch in pbar:
        ...
        with torch.cuda.amp.autocast():
            preds = model(images)                 # no labels during inference
            losses = criterion(preds, targets)
...
def main():
    ...
    parser.add_argument('--save_dir', type=str, default='checkpoints_guizhou_emb_dinov2_lora_enhanced')
    ...
    parser.add_argument('--use_frequency', action='store_true', default=True)
    parser.add_argument('--no_frequency', dest='use_frequency', action='store_false')
    ...
    parser.add_argument('--defect_use_fused', action='store_true', default=False,
                        help='Use the fused feature (instead of cnn_vec) as input to the defect head (cls mode only)')
    ...
    print(f'Defect head input: {"fused" if args.defect_use_fused else "cnn_vec"}')
    ...
    common_model_kwargs = {
        ...
        'defect_use_fused': args.defect_use_fused,
    }
    ...
    if args.resume and os.path.exists(args.resume):
        ...
        if isinstance(checkpoint, dict) and 'model_state_dict' in checkpoint:
            model = EmbroideryNetEnhanced(**common_model_kwargs).to(device)
