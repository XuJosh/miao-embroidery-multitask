...
def plot_loss_curves():
    """Plot training/validation loss curves for ablation variants."""
    variants = {
    ...
    for idx, (name, path) in enumerate(variants.items()):
        ...
        ax = axes[idx]
        ax.plot(epochs, train_total, 'r-', alpha=0.4, label='train loss')
        ax.plot(epochs, val_total, color='#FF7F0E', alpha=0.4, label='val loss')
        ax.plot(epochs, smooth(train_total), 'g--', linewidth=2, label='smooth train loss')
        ax.plot(epochs, smooth(val_total), color='#8B4513', linestyle='--', linewidth=2, label='smooth val loss')
    ...
    plt.suptitle('Training and Validation Loss Curves of Ablation Variants', fontsize=16, y=1.02)
    plt.tight_layout()
    out_path = 'D:/song/kimi_DEMO/blue_calico_multitask/ablation_loss_curves.png'
