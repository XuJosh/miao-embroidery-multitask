#!/usr/bin/env bash
# Combined ResNet50 + DINOv2/Transformer model where auth/defect also use fused features.
...
SAVE_DIR="checkpoints_combined_fused_correct"
LOG_FILE="logs/train_combined_fused_correct_150.log"
...
PY
        ...
        if [ "$EPOCH" -ge "$EPOCHS" ]; then
            echo "[$(date)] Combined fused model already completed $EPOCH epochs. Exiting."
            break
        fi
        echo "[$(date)] Attempt $attempt: resuming combined fused from $RESUME (epoch $EPOCH)"
    else
        echo "[$(date)] Attempt $attempt: starting combined fused from scratch"
    fi
    ...
    if [ "$EXIT_CODE" -eq 0 ]; then
        echo "[$(date)] Combined fused training exited cleanly."
        break
    fi

    echo "[$(date)] Combined fused exited with code $EXIT_CODE. Restarting in 30 seconds..."
    sleep 30
done
