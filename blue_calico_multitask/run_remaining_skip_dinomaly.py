"""
Skip Dinomaly and the arXiv DINOv3 experiments.
Run the remaining published-2025 comparison experiments sequentially:
    1. MobileNetV4 60-epoch evaluation (already trained)
    2. MambaOut-Small (CVPR 2025) 50 epoch + eval
    3. MambaOut-Tiny (CVPR 2025) 50 epoch + eval
    4. UniNet (CVPR 2025) anomaly-detection training with custom loop
"""
import os
import subprocess
...
def main():
    steps = [
        # 1) Evaluate MobileNetV4 60-epoch checkpoints
        ([PYTHON, "evaluate_external_checkpoints.py",
          ...
          "--save_dir", "checkpoints_external_mobilenetv4_conv_small.e1200_r224_in1k_correct",
          "--batch_size", "64",
          "--output", "mobilenetv4_60ep_test_results.json"],
         "mobilenetv4_60ep_eval.log"),

        # 2) MambaOut Small 50ep + eval
        ...
        ([PYTHON, "evaluate_external_checkpoints.py",
          "--backbone", "mambaout_small.in1k",
          "--data_dir", "data/guizhou_embroidery_correct",
          "--save_dir", "checkpoints_external_mambaout_small.in1k_correct",
          "--batch_size", "64",
          "--output", "mambaout_small_test_results.json"],
         "mambaout_small_eval.log"),

        # 3) MambaOut Tiny 50ep + eval
        ([PYTHON, "train_external.py",
        ...
        # 4) UniNet anomaly detection (custom loop to avoid Lightning hang)
