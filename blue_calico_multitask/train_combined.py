"""
Training script for EmbroideryNetCombined.
ResNet50 handles auth/defect; ResNet50 + DINOv2/Transformer fusion handles pattern.
"""
...
from model_combined import EmbroideryNetCombined, DynamicMultiTaskLoss
    ...
    for batch in pbar:
        ...
        if scaler is not None:
            with torch.cuda.amp.autocast():
                preds = model(images, labels=targets)
...
def main():
    ...
    parser.add_argument('--resnet_lr', type=float, default=1e-4)
    parser.add_argument('--lora_lr', type=float, default=1e-4)
    parser.add_argument('--input_size', type=int, default=224)
    parser.add_argument('--defect_mode', type=str, default='cls', choices=['cls'])
    ...
    parser.add_argument('--save_dir', type=str, default='checkpoints_combined_correct')
    ...
    parser.add_argument('--use_frequency', action='store_true', default=False)
    ...
    parser.add_argument('--use_dynamic_loss', action='store_true', default=True)
    parser.add_argument('--no_dynamic_loss', dest='use_dynamic_loss', action='store_false')
    parser.add_argument('--auth_defect_use_fused', action='store_true', default=False,
                        help='Use fused features (instead of ResNet50 vector) for auth/defect heads')
    parser.add_argument('--use_patch_transformer', action='store_true', default=True,
                        help='Use Patch Transformer branch over DINOv2 patch tokens')
    parser.add_argument('--no_patch_transformer', dest='use_patch_transformer', action='store_false',
                        help='Ablation: remove Patch Transformer branch')
    ...
    print(f'Frequency branch: {args.use_frequency}, ArcFace: {args.use_arcface}, Dynamic loss: {args.use_dynamic_loss}')
    print(f'Auth/defect input: {"fused" if args.auth_defect_use_fused else "resnet_vec"}')
    ...
    if args.resume and os.path.exists(args.resume):
        ...
        if isinstance(checkpoint, dict) and 'model_state_dict' in checkpoint:
            model = EmbroideryNetCombined(**common_model_kwargs).to(device)
    ...
    criterion = DynamicMultiTaskLoss(
        w_auth=args.w_auth, w_pattern=args.w_pattern, w_defect=args.w_defect,
        defect_mode=args.defect_mode, use_dynamic=args.use_dynamic_loss,
    ).to(device)
    ...
    optimizer = optim.Adam([
        ...
        {'params': resnet_params, 'lr': args.resnet_lr},
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        if args.use_dynamic_loss and hasattr(criterion, 'log_vars'):
            print(f"  -> loss weights (auth/pat/def): "
                  f"{torch.exp(-criterion.log_vars).detach().cpu().tolist()}")
